...
void bubbleSort()
{
    int i, j;
    bool swapped;
    for (i = 0; i < numElem - 1; i++) // Outer loop for passes
    {
        swapped = false;
        for (j = 0; j < numElem - i - 1; j++) // Inner loop for comparisons
        {
            if (myArray[j] > myArray[j + 1]) // Check if the current element is greater than the next
            {
                swap(myArray[j], myArray[j + 1]); // Swap if elements are in the wrong order
                swapped = true;
            }
        }
...
// An element is successfully inserted at the beginning! 
// Showing elements in my array 
// 3 | 3 | 7 | 7 | 9 | 9 | 11 | 15 | 19 | 20 | 37 | 128 | 256 | 
